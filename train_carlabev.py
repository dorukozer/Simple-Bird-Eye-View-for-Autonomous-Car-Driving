...
def run_model(model, loss_fn, d, device='cuda:0', sw=None, global_step=0, type='train'):
    metrics = {}
    total_loss = torch.tensor(0.0, requires_grad=True).to(device)

    imgs, rots, trans, intrins, lrtlist_velo, vislist, tidlist, scorelist, seg_bev_g, valid_bev_g, center_bev_g, offset_bev_g, radar_data, egopose = d

    B0,T,S,C,H,W = imgs.shape
    assert(T==1)

    #temp = valid_bev_g.clone()
    #valid_bev_g = 1 - seg_bev_g #torch.zeros_like(temp) #1 - seg_bev_g #.clone() #torch.ones_like(temp)
    ...
    #print('RGB CAM SAHPE ', rgb_camXs.shape)
    ...
    #print('MODEL OPUT seg ', type(seg_bev_e) , seg_bev_e.shape)
    #print('MODEL OPUT seg g ', type(seg_bev_g) , seg_bev_g.shape)
    ...
    intersection = (seg_bev_e_round*seg_bev_g).sum(dim=[1,2,3])*valid_bev_g
    union = ((seg_bev_e_round+seg_bev_g)).clamp(0,1).sum(dim=[1,2,3])*valid_bev_g
    ...
    if global_step == 1:
            ...
            for i in range(rgb_camXs.shape[1]):
                ...
                plt.savefig(f"/home/mbarin/Desktop/vs-code/simple-bev/simple_bev/logs_carlabev/rgb_cam/{global_step}_{type}_cam{i}.png")
                #plt.show()
            ...
            plt.savefig(f"/home/mbarin/Desktop/vs-code/simple-bev/simple_bev/logs_carlabev/segmentation_gt/{global_step}_{type}.png")
            #plt.show() 
            ...
            plt.savefig(f"/home/mbarin/Desktop/vs-code/simple-bev/simple_bev/logs_carlabev/valid_gt/{global_step}_{type}.png")
            #plt.show() 
    ...
    if global_step % 10 == 0:
        ...
        plt.savefig(f"/home/mbarin/Desktop/vs-code/simple-bev/simple_bev/logs_carlabev/segmentation_pred/{global_step}_{type}.png")
        #plt.show()
...
def main(
        ...
        data_dir='/home/mbarin/storage/data-transfuser/',
        ...
        rand_crop_and_resize= False, #True,
        ncams=6,
        nsweeps=3,
        # model
        encoder_type='res101',
    ...
    # set up dataloaders
    final_dim = (int(160 * res_scale), int(320 * res_scale))
    ...
    #sample = next(iter(train_dataloader)) # B ? NUM_CAM C H W  --> comes from torch.stack. adds dimension
    
    # print('TRAIN  DATA ' , type(sample))
    # print('TRAIN  DATA ' , len(sample))
    # print('TRAIN  DATA ' , sample[0].shape)
    ...
    while global_step < max_iters:
        ...
        for internal_step in range(grad_acc):
            ...
            total_loss, metrics = run_model(model, seg_loss_fn, sample, device, sw_t,global_step)
        ...
        if do_val and (global_step) % val_freq == 0:
            ...
            with torch.no_grad():
                total_loss, metrics = run_model(model, seg_loss_fn, sample, device, sw_v, global_step, type='val')
